  ...
  def stats_store
    ...
    if is_number?(lollipop.last.split('-').last)
      ...
      if lollipop[n-2] == "type"
        session[:stored_type] = lollipop.last.split('%20').join(' ').split('%C3%A9').join('é')
        session[:stored_ville] = nil
      elsif lollipop[n-2] == "ville"
        session[:stored_ville] = lollipop.last.split('%20').join(' ').split('%C3%A9').join('é')
        session[:stored_type] = nil
      else
        session[:stored_type] = lollipop[n-2].split('%20').join(' ').split('%C3%A9').join('é')
        session[:stored_ville] = lollipop.last.split('%20').join(' ').split('%C3%A9').join('é')
      end
    else
